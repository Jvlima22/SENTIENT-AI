  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        {/* category marquee — 6 cópias garantem que a faixa sempre seja mais larga que
            a tela (mesmo em monitores ultrawide), então o loop nunca "acaba": o -16.6667%
            (1/6 da largura total) fecha exatamente no início do próximo conjunto */}
        {categories.length > 0 && (
          <div className="relative z-10 border-t border-white/10 py-5 marquee-wrap overflow-hidden">
            <div className="flex gap-3 w-max badges-track">
              {[...categories, ...categories, ...categories, ...categories, ...categories, ...categories].map((c, i) => {
      ...
      <section id="catalogo" className="max-w-[1400px] mx-auto px-5 md:px-8 py-8 scroll-mt-20">
        ...
        <div className="flex items-center gap-2 mb-10 overflow-x-auto no-scrollbar -mx-5 px-5 md:-mx-8 md:px-8 lg:mx-0 lg:px-0 lg:flex-wrap">
          <DropdownMenu>
          ...
          <span className="mx-1 w-px h-5 bg-white/10 flex-shrink-0" />
...
const Chip = ({ active, onClick, children, testid }) => (
  <button onClick={onClick} data-testid={`filter-${testid}`}
    className={`text-sm px-4 py-1.5 rounded-full border transition-colors flex-shrink-0 whitespace-nowrap ${
