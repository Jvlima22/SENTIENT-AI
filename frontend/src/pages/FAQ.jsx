  ...
  return (
    <div className="max-w-3xl mx-auto px-5 md:px-8 py-12">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs uppercase tracking-wide text-[#00F0FF] font-mono-code">Ajuda</span>
      </div>
        ...
        <Accordion type="single" collapsible className="space-y-3" data-testid="faq-accordion">
          {faqs.map((f) => (
            <AccordionItem key={f.id} value={f.id} data-testid={`faq-${f.id}`}
              ...
              <AccordionTrigger className="text-left font-display text-base hover:no-underline hover:text-[#00F0FF] transition-colors py-5">
                {f.question}
              </AccordionTrigger>
