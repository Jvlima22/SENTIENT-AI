  ...
  return (
    <div className="max-w-[1100px] mx-auto px-5 md:px-8 py-12">
      <div className="flex items-center gap-4 mb-10">
        <div className="w-16 h-16 rounded-full bg-[#00F0FF]/10 border border-[#00F0FF]/30 flex items-center justify-center overflow-hidden">
          {user.picture ? <img src={user.picture} alt="" className="w-full h-full object-cover" /> : <User className="w-8 h-8 text-[#00F0FF]" />}
        </div>
        <div>
      ...
      <Tabs defaultValue="downloads" className="w-full">
        <TabsList className="bg-white/5 border border-white/10 rounded-full p-1 mb-8">
          <TabsTrigger value="downloads" data-testid="tab-downloads" className="rounded-full data-[state=active]:bg-[#00F0FF] data-[state=active]:text-black px-5">
            <Download className="w-4 h-4 mr-2" /> Downloads
          </TabsTrigger>
          <TabsTrigger value="purchases" data-testid="tab-purchases" className="rounded-full data-[state=active]:bg-[#00F0FF] data-[state=active]:text-black px-5">
            <ShoppingBag className="w-4 h-4 mr-2" /> Compras
          </TabsTrigger>
          <TabsTrigger value="profile" data-testid="tab-profile" className="rounded-full data-[state=active]:bg-[#00F0FF] data-[state=active]:text-black px-5">
            ...
            <TabsContent value="downloads" data-testid="downloads-panel">
              {downloads.length === 0 ? <Empty text="Nenhum download ainda. Explore o marketplace!" /> : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {downloads.map((d) => <HistoryRow key={d.id} item={d} action={<a href={d.download_url} target="_blank" rel="noreferrer" className="text-[#00F0FF] flex items-center gap-1 text-sm hover:underline"><Download className="w-4 h-4" /> Baixar</a>} />)}
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="profile" data-testid="profile-panel">
              <div className="max-w-md space-y-4 rounded-2xl glass p-6 border border-white/10">
                ...
                <button onClick={saveProfile} disabled={saving} data-testid="profile-save"
                  className="bg-[#00F0FF] text-black font-medium px-6 py-2.5 rounded-full hover:bg-white transition-colors flex items-center gap-2 disabled:opacity-60">
...
const HistoryRow = ({ item, action }) => (
  <div className="flex items-center gap-4 rounded-xl bg-[#0F0F13] border border-white/10 p-3">
    ...
    <div className="flex-1 min-w-0">
      <Link to={`/produto/${item.product_id}`} className="font-display text-sm hover:text-[#00F0FF] transition-colors line-clamp-1">{item.product_title}</Link>
...
const Empty = ({ text }) => (
  <div className="text-center py-16">
    ...
    <Link to="/" className="text-[#00F0FF] hover:underline inline-flex items-center gap-1">Ir ao marketplace <ExternalLink className="w-4 h-4" /></Link>
  </div>
);
