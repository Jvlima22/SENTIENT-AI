...
function SkillDialog({ skill, copied, onCopy, onClose }) {
  ...
  const save = async () => {
    ...
    } catch { toast.error("Não foi possível atualizar sua coleção."); }
  };
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#101015] sm:p-4 sm:bg-black/75 sm:backdrop-blur-sm" onMouseDown={onClose}>
      <section onMouseDown={(e) => e.stopPropagation()} className="flex h-[100dvh] w-full max-w-none flex-col overflow-hidden bg-[#101015] sm:h-auto sm:max-h-[calc(100dvh-2rem)] sm:max-w-4xl sm:rounded-2xl sm:border sm:border-white/15 sm:shadow-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5 md:p-6">
          <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6"><p className="text-white/60 max-w-2xl">{skill.description}</p><button onClick={save} className={`inline-flex w-full sm:w-auto justify-center h-fit items-center gap-2 rounded-full border px-4 py-2.5 text-sm ${saved ? "border-[#1689E8] text-[#72baff]" : "border-white/15 text-white/70"}`}><Bookmark className={`w-4 h-4 ${saved ? "fill-current" : ""}`} />{saved ? "Na sua coleção" : "Adicionar à coleção"}</button></div>
...
function McpTab({ skill }) {
  ...
  return <div className="space-y-5"><div className="rounded-xl border border-white/10 bg-black/25 p-4 sm:p-5"><div className="flex gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#1689E8]/15 text-[#72baff]"><Blocks className="w-5 h-5" /></div><div><h3 className="font-display text-base">Conecte ferramentas à sua IA</h3><p className="text-sm text-white/55 mt-1">A Sentient AI não armazena suas chaves. A configuração é feita na IA ou editor que você escolher.</p></div></div><div className="mt-5"><p className="text-xs uppercase tracking-[0.12em] text-white/45 mb-2">Recomendados para esta skill</p><div className="flex flex-wrap gap-2">{recommended.map((name) => <span key={name} className="rounded-full border border-[#1689E8]/30 bg-[#1689E8]/10 px-3 py-1.5 text-xs text-[#9bcfff]">{name}</span>)}</div></div></div><div><p className="text-xs uppercase tracking-[0.12em] text-white/45 mb-3">Onde configurar</p><div className="grid grid-cols-1 min-[380px]:grid-cols-2 gap-2">{Object.keys(setup).map((name) => <button key={name} onClick={() => setClient(name)} className={`rounded-xl border px-3 py-2.5 text-left text-xs ${client === name ? "border-[#1689E8] bg-[#1689E8]/10 text-white" : "border-white/10 text-white/55 hover:border-white/30"}`}>{name}</button>)}</div><p className="mt-3 rounded-xl border border-white/10 bg-black/25 p-3 text-sm text-white/65">{setup[client]}</p></div><div><div className="flex justify-between gap-3 mb-2"><p className="text-xs uppercase tracking-[0.12em] text-white/45">Modelo de configuração</p><button onClick={() => copy(config, "Configuração MCP copiada.")} className="text-xs text-[#FF7A59]">Copiar</button></div><pre className="overflow-x-auto rounded-xl border border-white/10 bg-black/45 p-4 text-xs leading-relaxed text-white/70 font-mono-code whitespace-pre-wrap">{config}</pre></div><div className="rounded-xl border border-[#FF7A59]/25 bg-[#FF7A59]/[0.06] p-4"><div className="flex gap-2"><ShieldCheck className="w-4 h-4 shrink-0 text-[#FF7A59] mt-0.5" /><div><p className="text-sm font-medium">Instrução segura para a IA</p><p className="mt-1 text-xs leading-relaxed text-white/60">Copie este contexto junto do prompt para orientar o uso das ferramentas.</p></div></div><button onClick={() => copy(context, "Instrução MCP copiada.")} className="mt-3 inline-flex w-full sm:w-auto justify-center items-center gap-2 rounded-full bg-[#FF7A59] px-3.5 py-2.5 text-xs font-medium text-black"><Copy className="w-3.5 h-3.5" />Copiar instrução</button></div></div>;
}
...
function LoginRequiredModal({ onClose }) { return <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70" onMouseDown={onClose}><section onMouseDown={(event) => event.stopPropagation()} className="w-full max-w-sm rounded-2xl border border-white/15 bg-[#15151b] p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div className="w-10 h-10 rounded-full bg-[#FF7A59]/15 text-[#FF7A59] flex items-center justify-center"><Bookmark className="w-5 h-5" /></div><button onClick={onClose} className="text-white/45 hover:text-white"><X className="w-5 h-5" /></button></div><h3 className="font-display text-xl mt-5">Salve suas skills</h3><p className="text-sm text-white/60 mt-2 leading-relaxed">Entre ou crie uma conta para adicionar prompts à sua coleção e acessá-los quando quiser.</p><div className="flex gap-3 mt-6"><button onClick={onClose} className="flex-1 rounded-full border border-white/15 px-4 py-2.5 text-sm text-white/70">Agora não</button><Link to="/login" className="flex-1 rounded-full bg-[#FF7A59] px-4 py-2.5 text-center text-sm font-medium text-black">Entrar</Link></div></section></div>; }
const AI_LOGOS = {
